...
[xv6を読む：メモリアロケータ http://yshigeru.hatenablog.com/entry/xv6を読む：メモリアロケータ]
- Memory Layout
    - カーネル
        - 仮想: KERNBASE + 0x100000 ~ end
        - 物理: 0x100000 ~ end - KERNBASE
        - endはリンク時に決まる
  物理: end - KERNBASEからPHYSTOP
  	ページとして利用できる
   この空間でフリーリストを構築する
   TODO: 図が間違っている。緑はtextのみ。
[https://gyazo.com/16ba99f651cc2f40252ce9d4d1d02262]
...
`run`の構造体が空いているページのリスト

- 2回にわける理由
    - entry.Sでは物理アドレス0~4MBしかマッピングしていない(物理アドレス0~4MBまでしか使えない)
    - kinit1()と後でkvmalloc()を読んで本来のページングを行っている
 ...
 `kfree()`
    ...
    77 }
 	L64: エラーチェック
 		...
 		`v`がFree memory(Fig2.2): end~P2V(PHYSTOP)であるか
...
# Code: sbrk
  ...
  L54: メモリを増やす
   ...
   175 }
  L162: 実行中のプロセスの取得
  L165 - 167: 割り当てているメモリを増やす場合
  L168 - 171: 割り当てているメモリを減らす場合
 `deallocuvm()`
 	code:vm.c
  ...
  L266: 該当のPTEの捜索
  	...
  	もしなかった場合に`walkpagidr()`が割り当てることはしない(3個目の引数が0であるため)
...
# Code: exec
   ...
   106  bad:
	...
	L43 - 58: ELFのセグメント毎にメモリにロード
 	...
 	L56:  `ip`(inode)をELFを元に所定の仮想アドレスに展開
